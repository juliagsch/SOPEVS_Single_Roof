...
double sim(vector<double> &load_trace, vector<double> &solar_trace, size_t start_index, size_t end_index, double cells, double pv, double b_0, std::vector<EVRecord> evRecords, std::vector<std::vector<EVStatus>> allDailyStatuses, double max_soc, double min_soc, int Ev_start, bool printGridCost)
{
	...
	for (int day = 0; day < (end_index - start_index) / 24; day++)
	{
		...
		for (int hour = 0; hour < 24; hour++)
		{
			...
			double hourly_load = 0.0;
			// Always charge if EV is not fully charged yet
			if (Operation_policy == "safe")
			{
				if (isHome == true)
				{
				...
				hourly_load = load_trace[index_t_load] + maxCharging;
				total_load += hourly_load;
			...
			// Charge only during LBN hours
			else if (Operation_policy == "lbn")
			{
				if (isHome == true && hour < 5)
				{
					maxCharging = get_maxCharging(ev_b);
				}
			...
			else if (Operation_policy == "sunlight")
			{
				if (isHome == true && hour >= 9 && hour <= 15)
				{
					maxCharging = get_maxCharging(ev_b);
				}
			...
			else if (Operation_policy == "sunlight_lbn")
			{
				if (isHome == true && ((hour >= 9 && hour <= 15) || hour < 5))
				{
					maxCharging = get_maxCharging(ev_b);
				}
			...
			else if (Operation_policy == "soctarget")
			{
			...
			else if (Operation_policy == "lbn_soctarget")
			{
				// Check if EV should currently be charging
				isCharging = get_is_charging(allDailyStatuses[ev_day], ev_b, hour);

				if (isHome == true && (hour < 5 || isCharging))
				{
					maxCharging = get_maxCharging(ev_b);
				}
			...
			else if (Operation_policy == "sunlight_soctarget")
			{
				// Check if EV should currently be charging
				isCharging = get_is_charging(allDailyStatuses[ev_day], ev_b, hour);

				if (isHome == true && ((hour >= 9 && hour <= 15) || isCharging))
				{
					maxCharging = get_maxCharging(ev_b);
				}
			...
			else if (Operation_policy == "sunlight_lbn_soctarget")
			{
				// Check if EV should currently be charging
				isCharging = get_is_charging(allDailyStatuses[ev_day], ev_b, hour);

				if (isHome == true && ((hour < 5 || (hour >= 9 && hour <= 15)) || isCharging))
				{
					maxCharging = get_maxCharging(ev_b);
				}
			...
			last_soc = ev_b;
			total_load_vec.push_back(hourly_load);
	...
	{
		for (const auto &value : total_load_vec)
		{
		...
		outputFile.close();
		std::cout << "Successfully wrote vector values to " << out_path << std::endl;
	}
	else
	{
		std::cerr << "Unable to open file: " << out_path << std::endl;
	}
